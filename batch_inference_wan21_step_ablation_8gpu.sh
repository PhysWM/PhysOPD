...
#
# 常用用法:
#   bash batch_inference_wan21_step_ablation_8gpu.sh
#   NUM_GPUS=4 PINN_STEP_RANGE=41-50 bash batch_inference_wan21_step_ablation_8gpu.sh
#   GPU_IDS=0,2,5 PINN_STEP_RANGE=31-50 bash batch_inference_wan21_step_ablation_8gpu.sh
#   OUTPUT_DIR=output_videos_test CHECKPOINT_PATH=xxx.pt PINN_STEP_RANGE=1-50 bash batch_inference_wan21_step_ablation_8gpu.sh
#
# 环境变量:
...
#   MODEL_ID           模型 ID
#   PINN_STEP_RANGE    PINN 生效的 denoising step，如 41-50 / 31-50 / 1-50 / none
...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/phygenbench_videos_wan21_pinn_step_ablation}"
...
PINN_STEP_RANGE="${PINN_STEP_RANGE:-all}"
...
run_one() {
    ...
    local cmd=(
        "${PYTHON_BIN}"
        examples/wanvideo/pinn_inference/batch_inference_pinn_step_ablation.py
        "${COMMON_ARGS[@]}"
        --start_id "${start_id}"
        --end_id "${end_id}"
        "$@"
...
echo "Model: ${MODEL_ID}"
echo "PINN step range: ${PINN_STEP_RANGE}"
if [[ "${DISABLE_PROMPT_REFINEMENT}" == "0" ]]; then
